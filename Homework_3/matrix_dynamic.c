...
matrix transpose(matrix *m){
  ...
  transposedMatrix = create_empty(m->col_dim, m->row_dim); //switches rows with columns in size
  for(int i = 0; i < transposedMatrix.row_dim; i++){
    for(int j = 0; j < transposedMatrix.col_dim; j++){
    
      transposedMatrix.element[i][j] = m->element[j][i]; //fills in the information for the rows and the colums.
